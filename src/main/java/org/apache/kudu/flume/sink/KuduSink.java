...
/**
 * A Flume sink that reads events from a channel and writes them to a Kudu
 * table.
 *
 * <p>
 * <strong>Flume Kudu Sink configuration parameters</strong>
 *
 ...
 * <td></td>
 * <td>Yes</td>
 * <td>The name of the Flume channel to read.</td>
 * </tr>
 * <tr>
 * <td>type</td>
 * <td></td>
 * <td>Yes</td>
 * <td>Component name. Must be {@code org.apache.kudu.flume.sink.KuduSink}</td>
 * </tr>
 * <tr>
 * <td>masterAddresses</td>
 * <td></td>
 * <td>Yes</td>
 * <td>Comma-separated list of "host:port" Kudu master addresses. The port is
 * optional.</td>
 * </tr>
 * <tr>
 * <td>tableName</td>
 * <td></td>
 * <td>Yes</td>
 * <td>The name of the Kudu table to write to.</td>
 * </tr>
 * <tr>
 * <td>batchSize</td>
 * <td>100</td>
 * <td>No</td>
 * <td>The maximum number of events the sink takes from the channel per
 * transaction.</td>
 * </tr>
 * <tr>
 ...
 * <td>true</td>
 * <td>No</td>
 * <td>Whether to ignore duplicate primary key errors caused by inserts.</td>
 * </tr>
 * <tr>
 * <td>timeoutMillis</td>
 * <td>10000</td>
 * <td>No</td>
 * <td>Timeout period for Kudu write operations, in milliseconds.</td>
 * </tr>
 * <tr>
 * <td>producer</td>
 * <td>{@link SimpleKuduOperationsProducer}</td>
 * <td>No</td>
 * <td>The fully-qualified class name of the {@link KuduOperationsProducer} the
 * sink should use.</td>
 * </tr>
 * <tr>
 * <td>producer.*</td>
 * <td></td>
 * <td>(Varies by operations producer)</td>
 * <td>Configuration properties to pass to the operations producer
 * implementation.</td>
 * </tr>
 * </table>
 *
 ...
 *
 * <p>
 * After building the sink, in order to use it with Flume, place the file named
 ...
 *
 * <p>
 * For detailed instructions on using Flume's plugins.d mechanism, please see
 * the plugins.d section of the <a href=
 * "https://flume.apache.org/FlumeUserGuide.html#the-plugins-d-directory">Flume
 * User Guide</a>.
 */
...
public class KuduSink extends AbstractSink implements Configurable {
	...
	public void configure(Context context) {
		...
		Preconditions.checkNotNull(masterAddresses, "Missing master addresses. Please specify property '$s'.",
				MASTER_ADDRESSES);
		...
		interval = context.getLong("interval", DEFAULT_INTERVAL);
		...
		// Check for operations producer, if null set default operations
		// producer type.
		...
		try {
			Class<? extends KuduOperationsProducer> clazz = (Class<? extends KuduOperationsProducer>) Class
					.forName(operationProducerType);
	...
	public Status process() throws EventDeliveryException {
		if (session.hasPendingOperations()) {
			// If for whatever reason we have pending operations, refuse to
			// process
			// more and tell the caller to try again a bit later. We don't want
			// to
		...
		try {
			...
			while (txnEventCount < batchSize && curTime < endTime) {
			...
			if (responses != null) {
				for (OperationResponse response : responses) {
					// Throw an EventDeliveryException if at least one of the
					// responses was
					// a row error. Row errors can occur for example when an
					// event is inserted
					// into Kudu successfully but the Flume transaction is
					// rolled back for some reason,
					// and a subsequent replay of the same Flume transaction
					// leads to a
					// duplicate key error since the row already exists in Kudu.
					// Note: Duplicate keys will not be reported as errors if
					// ignoreDuplicateRows
